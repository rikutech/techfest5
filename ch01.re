...
きました、真骨頂です。関数型言語は同じ引数に対しては必ず同じ返り値になります。(ユーザー入力やDB等除く)
...
愛のままに書いた乱文でしたが、これを読んでElixirを始めてくれる方が一人でもいたらめちゃくちゃ嬉しいです。Twitter(@rikutech)にリプライください。
泣いて喜びます。ランチおごります。

なお、文中で度々Ruby・Railsと比較をしましたが、依然として僕はRubyもRailsも好きで、使用する大きなメリットがあると思います。
単純なプログラムにおいては状態を持たせてガシガシコードを書くほうが早いと考えていますし、何より「全てがオブジェクト」「書いていて楽しい」等の理念が気に入っています。
あとはコミュニティの大きさですね。やはりRubyコミュニティの大きさは他新興言語と比較しがたく、欲しい情報やGemがなんでも揃います。
